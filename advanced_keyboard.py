...
class MediaPipeHandTracker:
    ...
    def _process_detected_hands(self, frame: np.ndarray, results) -> None:
        ...
        for hand_landmarks, handedness in zip(results.multi_hand_landmarks, results.multi_handedness):
            ...
            detected_hand = DetectedHand(landmarks, hand_label, confidence)
            
            # Assign to appropriate hand (note: MediaPipe labels are from camera perspective)
            if hand_label == "Right":
    ...
    def _update_cursor_from_right_hand(self, frame: np.ndarray) -> None:
        ...
        # Map to keyboard area (consider the GUI layout)
        # Camera feed will be on left side, keyboard on right side
        keyboard_x = int((finger_x / frame_w) * (WINDOW_WIDTH * 0.6)) + int(WINDOW_WIDTH * 0.4)
